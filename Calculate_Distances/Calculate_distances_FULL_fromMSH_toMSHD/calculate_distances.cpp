...
**                           CALCULATE_DISTANCES                 **
**                                                               **
** Program: calculate_distances.cpp                              **
** Version: 1.2                                                  **
**    {interface_effSFaces first part} - > from .msh and defs.tmp**
**        get vertex/defect distances with a chosen algorithm    **
**        and store info in tri_.mshd file                       **
** (with common.h)                                               **
** Last Change: Sep 13 2018                                      **
** Now add faces to defects                                      **
**                                                               **
*******************************************************************/
#include <iostream>
#include <fstream>
...
/*******************************************************************/
/*
void init_raw_input()         // change to C++ program arguments
{
 // choose algorithm for surface distance calculations
	...
	// Add defect effect
    printf("Input defect file:  ");   //+++ defs
	scanf("%s",fD_name);            //+++ defs
}*/
...
void init(int argCount, char **argList)         // change to C++ program arguments
{
    if(argCount < 5)
    {
        printf("\n\n Input Params can be passed as program command-line arguments (4 values).\n\n Here input by hand:\n");
      // choose algorithm for surface distance calculations
        printf("  Choose distance calculation method \n\t1.Circle\t2.Dijkstra\t3.Exact\t\t5.Euclidean\n");
        printf("  Input (int):  ");
        scanf("%d", &distance_method);
    ...
    {
        char *dumm;
        printf("\n\n Reading input params as: 1.DistanceMethod, 2.MeshFileName, 3.MeshID, 4.DefectFileName\n\n");
   ...
   // Write ou file with params and progress
    ...
    f_ou_name = "ou_ico"+std::to_string(mesh_number)+"_"+std::to_string(distance_method)+".dat";
...
{
    long i, nn,j;
    double x2,y2,z2,x0,y0,z0,norm;
    double mean_distance, distance_difference;

    for(i=0; i<num_of_meshpoint; i++){
        nn = vertex[i].num_of_neighbors;
        if(nn==5){
        // Calculate distance to point and get position unitary vector
            mean_distance = 0.;
            norm = sqrt(vertex[i].x*vertex[i].x+vertex[i].y*vertex[i].y+vertex[i].z*vertex[i].z);
            x0 = vertex[i].x/norm;
            y0 = vertex[i].y/norm;
            z0 = vertex[i].z/norm;
            mean_distance += norm;
          // Calculate neighbors distances to center and average
            for(j=0; j<nn; j++){
                x2 = vertex[vertex[i].neighbor[j]].x;
                x2 = x2*x2;
                y2 = vertex[vertex[i].neighbor[j]].y;
                y2 = y2*y2;
                z2 = vertex[vertex[i].neighbor[j]].z;
                z2 = z2*z2;
                mean_distance += sqrt(x2+y2+z2);
            }
            mean_distance /= nn+1;
           // Change position such that distance fits average distance (including old position)
            distance_difference = norm-mean_distance;
            vertex[i].x = vertex[i].x - x0*distance_difference/2.;
            vertex[i].y = vertex[i].y - y0*distance_difference/2.;
            vertex[i].z = vertex[i].z - z0*distance_difference/2.;
            printf("\t\tPosition of vertex %ld changed by %.10G \n",i,distance_difference);
        }
    }
}*/
...
void calculate_defect_distance()
{
  ...
  //  Include progress bar on ou.dat to check remotely
    ...
    case 1: // circle
        ...
        for (i=0; i<num_of_meshpoint; i++){
            for(d=0;d<num_of_defects;d++){
                distance_to_defect = great_circle_distance(i,d);        //*****
                r2 = distance_to_defect*distance_to_defect;
                vertex[i].distToDefect[d] = distance_to_defect;   //LLLL
            }
            progress_bar(i, num_of_meshpoint);
            progress_bar_count = progress_bar_on_ou(i, num_of_meshpoint, progress_bar_count);
        }
